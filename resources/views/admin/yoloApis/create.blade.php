...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.yoloApi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.yolo-apis.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                 <label class="required" for="url">{{ trans('cruds.yoloApi.fields.api_name') }}</label>
                 <input type="text" class="form-control {{ $errors->has('api_name') ? 'is-invalid' : '' }}" placeholder="API Name" name="api_name" id="api_name" value="{{ old('api_name', '') }}" required  >
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('enviroment') ? 'is-invalid' : '' }}" name="enviroment_id" id="enviroment_id" required>
                    @foreach($enviroments as $id => $entry)
                        <option value="{{ $id }}" {{ old('enviroment_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('api_type') ? 'is-invalid' : '' }}" name="api_type" id="api_type" required>
                    ...
                    @foreach(App\Models\YoloApi::API_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('api_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="url">{{ trans('cruds.yoloApi.fields.url') }}</label>
                 <input type="text" class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" placeholder="URL" name="url" id="url" value="{{ old('url', '') }}" required readonly="readonly">
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('endpoint') ? 'is-invalid' : '' }}" type="text" name="endpoint" id="endpoint" value="{{ old('endpoint', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cognito') ? 'is-invalid' : '' }}" type="text" name="cognito" id="cognito" value="{{ old('cognito', '') }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('request_body') ? 'is-invalid' : '' }}" name="request_body" id="request_body" required>{{ old('request_body') }}</textarea>
